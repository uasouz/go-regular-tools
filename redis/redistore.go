...
func NewRediStore(size int, network, address, password string, keyPairs ...[]byte) (*RediStore, error) {
	return NewRediStoreWithPool(redis.NewClient(&redis.Options{
		MinIdleConns: size,
...
func NewRediStoreWithDB(size int, network string, address string, password string, DB int, keyPairs ...[]byte) (*RediStore, error) {
	return NewRediStoreWithPool(redis.NewClient(&redis.Options{
		MinIdleConns: size,
		PoolTimeout:  240 * time.Second,
		// TestOnBorrow: func(c redis.Conn, t time.Time) error {
...
func NewRediStoreWithPool(pool *redis.Client, keyPairs ...[]byte) (*RediStore, error) {
	rs := &RediStore{
...
func (s *RediStore) load(session *sessions.Session) (bool, error) {
	...
	// 	return false, err
	// }
	data, err := conn.Do("GET", s.keyPrefix+session.ID).Result()
	if err != nil {
		return false, err
	}
	if data == nil {
	...
	b, err := redis.NewStringCmd(data, err).Bytes()
	if err != nil {
		return false, err
	}
